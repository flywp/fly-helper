...
class PostLists {
    ...
    public static function for_post( WordPressGateway $wp, array $post ) {
        ...
        foreach ( $wp->post_terms( $post['id'] ) as $term ) {
            $lists[] = [ 'type' => 'term', 'id' => $term['id'], 'url' => $wp->term_link( $term['id'], $term['taxonomy'] ) ];
        }

        if ( 'post' !== $post['post_type'] ) {
            $lists[] = [ 'type' => 'archive', 'id' => null, 'url' => $wp->post_type_archive_link( $post['post_type'] ) ];

            return $lists;
        }

        $lists[] = [ 'type' => 'blog', 'id' => null, 'url' => HomeSource::blog_url( $wp ) ];
        $lists[] = [ 'type' => 'user', 'id' => (int) $post['author'], 'url' => $wp->author_link( $post['author'] ) ];

        if ( preg_match( '/^(\d{4})-(\d{2})/', (string) $post['date'], $date ) ) {
            $year  = (int) $date[1];
            $month = (int) $date[2];

            $lists[] = [ 'type' => 'date', 'id' => $year, 'url' => $wp->year_link( $year ) ];
            $lists[] = [ 'type' => 'date', 'id' => $year * 100 + $month, 'url' => $wp->month_link( $year, $month ) ];
        }
